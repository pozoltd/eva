...
class Cms implements ServiceProviderInterface
{
    ...
    {
        $contentBlocks = \Eva\ORMs\ContentBlock::active($this->app['zdb']);
        foreach ($contentBlocks as $contentBlock) {
            $items = json_decode($contentBlock->items);
            foreach ($items as &$item) {
                ...
                $item->__choices = $choices;
            }
            $contentBlock->items = json_encode($items);
        }
        return $contentBlocks;
    }

    public function contentBlocksValue($value)
    {
